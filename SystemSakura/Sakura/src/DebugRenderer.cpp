...
namespace Sakura{
	...
	void DebugRenderer::end(){
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ibo);
		// Orphan the buffer
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indicies.size() * sizeof(GLuint), nullptr, GL_DYNAMIC_DRAW);
		// Upload the data
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, m_indicies.size() * sizeof(GLuint), m_indicies.data());
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

		m_numElements = m_indicies.size();
		m_indicies.clear();
		m_verts.clear();
	}
	...
	void DebugRenderer::drawBox(const glm::vec4& destRect, const ColorRGBA8& color, float angle){
		...
		m_indicies.reserve(m_indicies.size() + 8);
	...
	void DebugRenderer::drawCircle(const glm::vec2& center, const ColorRGBA8& color, float radius){
		...
		// Set up indicies for index drawings
		m_indicies.reserve(m_indicies.size() + NUM_VERTS * 2);
		for (int i = 0; i < NUM_VERTS - 1; ++i){
			m_indicies.push_back(start + i);
			m_indicies.push_back(start + i + 1);
		}
		m_indicies.push_back(start + NUM_VERTS - 1);
		m_indicies.push_back(start);

	}

	void DebugRenderer::render(const glm::mat4& projectionMatrix, float lineWidth, std::string uniform){
		m_program.use();

		GLint pUniform = m_program.getUniformLocation(uniform);
