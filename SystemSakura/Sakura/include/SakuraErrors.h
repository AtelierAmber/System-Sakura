...
#define SAKURA_ASSERT_FUNC(funcT, str) __try{ funcT }__except(...){ fprintf(stderr, "Exception occured: %s\n", str.c_str()); fflush(stderr);}
...
#define SAKURA_PRINT_ERROR(err) fprintf(stderr, "%s\n", err); fflush(stderr);
...
namespace {
	...
	extern int sakura_static_assert(bool equate, const char* errStr, const char *file, int line){
		if (!equate){
			if (!isRecorded(std::string(errStr))){
				fprintf(stderr, "Exception occured: %s - %s:%i\n", errStr, file, line);
				fflush(stderr);
			}
			return 0;
		}
	...
	extern void fatalError(const std::string& errorString){
		if (!isRecorded(errorString)){
			fprintf(stderr, "Fatal Exception occured: %s\n", errorString.c_str());
			fflush(stderr);
